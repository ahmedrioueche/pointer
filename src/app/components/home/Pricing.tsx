    ...
    <div className={`flex flex-col p-6 bg-dark-background dark:bg-light-background rounded-lg shadow-md transition-transform transform hover:scale-105 hover:shadow-lg h-[430px]`}>
      <div className="flex flex-col flex-grow justify-between">
        <div className="flex flex-col items-center">
          <h3 className={`text-xl font-semibold mb-4 text-dark-text dark:text-light-text`}>
            {title}
          </h3>
          <p className={`text-4xl font-bold mb-4 text-dark-text dark:text-light-text`}>
            {price}
          </p>
        </div>
        <p className={`text-lg mb-6 text-dark-secondary dark:text-light-secondary text-center`}>
          {description}
        </p>
        <ul className="list-disc list-inside text-left mb-6">
          {features.map((feature, index) => (
            <li key={index} className={`flex items-center text-dark-text dark:text-light-text mb-2`}>
      ...
      <div className="flex justify-center mt-auto">
        <button className={`px-6 py-3 rounded-md ${isDarkMode ? 'bg-dark-primary text-dark-background hover:bg-dark-accent' : 'bg-light-primary text-light-background hover:bg-light-accent'} font-medium transition-colors duration-300`}>
          Choose Plan
        </button>
      </div>
    </div>
