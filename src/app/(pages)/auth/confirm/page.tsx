...
const Confirm: React.FC = () => {
    ...
    const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
        ...
        if (!childrenCount || isNaN(Number(childrenCount)) || Number(childrenCount) <= 0) {
            ...
            children.forEach((child, index) => {
                ...
                if (!child.age || isNaN(childAgeNum) || childAgeNum <= 0) {
                    ...
                    console.log("isNaN(childAgeNum)", isNaN(childAgeNum));
        ...
        if (Object.keys(currentErrors).length > 0) {
            ...
            for (const child of children) {
                ...
                const result = await apiInsertDB(childObject, childrenCount, '/api/auth/confirm');

                if (result.success) {
                    setStatus({ success: true, message: 'Your data has been saved successfully' })
    ...
    const handleChildrenCountChange = (e: ChangeEvent<HTMLInputElement>) => {
        ...
        console.log("Children Count Changed:", count); // Debugging statement
                       ...
                       <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex justify-center w-full mt-4">
                                <p className={`text-center ${resStatus.success ? 'text-green-400' : 'text-red-400'}`}>{resStatus.message}</p>
                            </div>
                            )}
                        <button
                            type="submit"
