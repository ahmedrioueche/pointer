...
const ChildCard: React.FC<CardProps> = ({ id, name, age, gender, achievedTasks, pendingTasks, icon, type, callback }) => {
  ...
  const handleRoutinesClick = (type : string) => {
    callback(id, type);
  };
                  ...
                  >
                    ...
                    {task.completionDate && (
                      <div className="flex items-center ml-auto">
                        <FaCalendarCheck className="text-dark-text mr-2" />
                        <span>{capitalizeFirstLetter(formatDateTime(new Date(task.completionDate)))}</span>
                      </div>
                    )}
                  </div>
                ))}
              ...
              <div className="flex flex-col items-center justify-between w-full mt-2 font-satisfy text-base">
                {pendingTasks.map((task, index) => (
                  ...
                  >
                    ...
                    {task.dueDate && (
                      <div className="flex items-center ml-auto">
                        <FaCalendarDay className="text-dark-text mr-2" />
                        <span>{capitalizeFirstLetter(formatDateTime(new Date(task.dueDate)))}</span>
                      </div>
                    )}
                  </div>
                ))}
      ...
      {type === "routines" && (
        <div className="mt-4 flex justify-between">
          <button
              className={`flex items-center mt-4 px-4 py-2 bg-white text-light-primary font-medium font-stix rounded-lg shadow-md hover:text-white transition-colors duration-300
              hover:bg-gradient-to-br ${gender === 'male' ? 'hover:from-blue-300 hover:to-blue-500' : 'hover:bg-gradient-to-br hover:from-pink-300 hover:to-pink-500'}`}
              onClick={() => handleRoutinesClick("daily")}
            >
            ...
            <button
              className={`flex items-center mt-4 px-4 py-2 bg-white text-light-primary font-medium font-stix rounded-lg shadow-md hover:text-white transition-colors duration-300
              hover:bg-gradient-to-br ${gender === 'male' ? 'hover:from-blue-300 hover:to-blue-500' : 'hover:bg-gradient-to-br hover:from-pink-300 hover:to-pink-500'}`}
              onClick={() => handleRoutinesClick("weekly")}
            >
              <FaCalendarAlt className="mr-2" /> Weekly
            </button>
        </div>
      )}
    </div>
